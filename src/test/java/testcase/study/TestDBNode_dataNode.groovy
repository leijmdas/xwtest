...
class TestDBNode_dataNode extends ITestImpl {
    ...
    void test0001_db_checkNum() {
        dbnode.sql("select count(*) as cnt from t_addr where id=1")
                .checkRecordNumber(1)
    }

    void test0002_db_checkRecord(){
        dbnode.sql("select * from t_addr where id=1").select().each {
            println it
        }
    ...
    void test0003_db_checkColumn() {
        dbnode.sql("select * from t_addr where id<=2").select().each {
            println it
        }
    ...
    void test0004_dataNode() {
        ...
        println jdbnode.sql("select * from t_user").getRecordNumber()
